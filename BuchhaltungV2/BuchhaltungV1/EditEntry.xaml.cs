...
namespace BuchhaltungV1
{
    ...
    {
        private Entry _originalEntry; //try not to override but can´t be a const for obvious reasons /gets changed when changing the num/
            ...
            if (NumInput.Text != "" && IdInput.Text != "" && AmountInput.Text != "" && OnTheHouseInput.Text != "")
            {
                foreach (Day d in Buchhaltung.CurrWeek.DaysInWeek)
                {
                    if (d.Name == Buchhaltung.CurrDay)
                    {
                        ...
                        {
                            if (IdInput.Text == Convert.ToString(e.ProductForEntry.Id) && AmountInput.Text == Convert.ToString(e.Amount))
                            {
